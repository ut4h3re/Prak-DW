...
function tampilkanDaftarBuku() {
    ...
    daftarBuku.forEach((buku, index) => {
        ...
        divBuku.classList.add('buku', 'bg-white', 'p-4', 'mb-2', 'rounded-lg', 'shadow-md');
        divBuku.innerHTML = `
            <p class="text-gray-800">${buku.tampilkanInfo()}</p>
            <button onclick="tambahKeFavorit(${index})" class="mt-2 mr-2 py-1 px-2 bg-blue-500 text-white rounded hover:bg-blue-600">Tambah ke Favorit</button>
            <button onclick="hapusBuku(${index})" class="mt-2 py-1 px-2 bg-red-500 text-white rounded hover:bg-red-600">Hapus</button>
        `;
...
function tampilkanBukuFavorit() {
    ...
    bukuFavorit.forEach((buku, index) => {
        ...
        divBuku.innerHTML = `
            <p class="text-gray-800">${buku.tampilkanInfo()}</p>
            <button onclick="hapusDariFavorit(${index})" class="mt-2 py-1 px-2 bg-red-500 text-white rounded hover:bg-red-600">Hapus</button>
        `;
